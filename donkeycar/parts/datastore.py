...
class OriginalWriter:
    ...
    def __init__(self, path):
        ...
        # self.out = cv2.namedWindow("Store")

        self.start_time = time.time()
        self.on = True
        self.pilot_subscriber = subscribers.PilotSubscriberThread(args = (self, "pilot", self.collector, False))
        self.image_subscriber = subscribers.ImageSubscriberThread(args = (self, "cam-image", self.collector, True), name = "StoreImage")
        self.mode_subscriber = subscribers.ModeSubscriberThread(args = (self, "mode", self.collector, False))
        self.recording_subscriber = subscribers.RecordingSubscriberThread(args = (self, "recording", self.collector, False))
        self.speed_subscriber = subscribers.SpeedSubscriberThread(args = (self, "teensy-speed", self.collector, False))
        self.pilot_subscriber.start()
        self.image_subscriber.start()
        self.mode_subscriber.start()
        self.recording_subscriber.start()
    ...
    def run(self):
        ...
        if self.recording:
            ...
            self.out['extra']['speed'] = self.speed
            self.out['extra']['target_speed'] = self.target_speed
            self.out['extra']['mode'] = self.mode
            self.out['extra']['collector'] = self.collector

            self.out['angle'] = self.angle
            self.out['trottle'] = self.throttle
...
class Tub(object):
    ...
    def run(self):
        ...
        if self.recording:
            ...
            json_data['milliseconds'] = self.record_time
            json_data['extra']['linaccel'] = None
            json_data['extra']['speed'] = self.speed
            json_data['extra']['target_speed'] = self.target_speed
            json_data['extra']['mode'] = self.mode
            json_data['extra']['collector'] = self.collector

            json_data['angle'] = self.angle
            json_data['trottle'] = self.throttle

            name = self.make_file_name('frame', ext = '.jpg')
            path = os.path.join(self.path, name)
            cv2.imwrite(path, self.frame)
            json_data['frame'] = name

            json_data['extra']['time'] = (time.time() - t) * 1000
